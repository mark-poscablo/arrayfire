...
template<typename inType, typename outType>
static
tuple<Array<outType>, Array<outType>>
meanvar(const Array<inType> &in, const Array<typename baseOutType<outType>::type>& weights,
    ...
    if(weights.isEmpty()) {
        ...
        normArr = createValueArray<outType>(meanArr.dims(), scalar<outType>(val));
    } else {
        meanArr = mean<outType, weightType>(input, weights, dim);
        Array<outType> wtsSum = cast<outType>(reduce<af_add_t, weightType, weightType>(weights, dim));
        Array<outType> ones = createValueArray<outType>(wtsSum.dims(), scalar<outType>(1));
    ...
    Array<outType> variance = arithOp<outType, af_mul_t>(normArr, redDiff, redDiff.dims());
...
template<typename inType, typename outType>
static
tuple<af_array, af_array>
meanvar(const af_array &in, const af_array &weights,
        const af_var_bias bias, const dim_t dim) {
...
template<typename inType, typename outType>
static Array<outType>
var(const Array<inType>& in,
...
af_err af_var(af_array *out, const af_array in, const bool isbiased, const dim_t dim)
{
    try {
        ...
        switch(type) {
            ...
            case u32: output = var_<uint  ,  float >(in, no_weights, bias, dim); break;
            case s16: output = var_<short ,  float >(in, no_weights, bias, dim); break;
            case u16: output = var_<ushort,  float >(in, no_weights, bias, dim); break;
            case s64: output = var_<intl  ,  double>(in, no_weights, bias, dim); break;
            case u64: output = var_<uintl ,  double>(in, no_weights, bias, dim); break;
            case  u8: output = var_<uchar ,  float >(in, no_weights, bias, dim); break;
...
af_err af_var_weighted(af_array *out, const af_array in, const af_array weights, const dim_t dim)
{
    try {
        ...
        switch(iType) {
            ...
            case u32: output = var_<uint  ,  float >(in, weights, AF_VARIANCE_POPULATION, dim); break;
            case s16: output = var_<short ,  float >(in, weights, AF_VARIANCE_POPULATION, dim); break;
            case u16: output = var_<ushort,  float >(in, weights, AF_VARIANCE_POPULATION, dim); break;
            case s64: output = var_<intl  ,  double>(in, weights, AF_VARIANCE_POPULATION, dim); break;
            case u64: output = var_<uintl ,  double>(in, weights, AF_VARIANCE_POPULATION, dim); break;
            case  u8: output = var_<uchar ,  float >(in, weights, AF_VARIANCE_POPULATION, dim); break;
...
af_err af_meanvar(af_array *mean, af_array *var, const af_array in,
  ...
  try {
      ...
      switch(iType) {
        ...
        case u32: tie(*mean, *var) = meanvar<uint,    float>(in, weights, bias, dim); break;
        case s16: tie(*mean, *var) = meanvar<short ,  float >(in, weights, bias, dim); break;
        case u16: tie(*mean, *var) = meanvar<ushort,  float >(in, weights, bias, dim); break;
        case s64: tie(*mean, *var) = meanvar<intl  ,  double>(in, weights, bias, dim); break;
        case u64: tie(*mean, *var) = meanvar<uintl ,  double>(in, weights, bias, dim); break;
        case  u8: tie(*mean, *var) = meanvar<uchar ,  float >(in, weights, bias, dim); break;
